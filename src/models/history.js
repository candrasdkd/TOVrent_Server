...
const getHistoryById = (id) => {
  return new Promise((resolve, reject) => {
    const queryString = `SELECT h.id AS id, h.user_id AS customerId, h.owner_id AS ownerId, v.picture AS picture, v.name AS vehicleName, v.type_id AS vehicleType, h.quantity AS totalQuantity, h.location AS vehicleLocation, h.price AS totalPrice, u.full_name AS customer, u.email AS email, u.phone_number AS phoneNumber, u.address AS address, h.booking_code AS bookingCode, h.days AS dayDuration, h.status_id AS orderStatus, u.card_number  AS cardNumber, h.method_payment AS paymentMethod, h.from_date AS startDate, h.to_date AS expiredDate FROM tb_histories h JOIN tb_users u ON u.id = h.user_id JOIN tb_users o ON o.id = h.owner_id JOIN tb_vehicles v ON v.id = h.vehicle_id WHERE h.id = ${id}`;
...
const getHistoryByUser = (params) => {
  return new Promise((resolve, reject) => {
    ...
    const queryString = `SELECT h.id AS id, h.user_id AS customerId, h.owner_id AS ownerId, v.picture AS picture, v.name AS vehicleName, v.type_id AS vehicleType, h.quantity AS totalQuantity, h.location AS vehicleLocation, h.price AS totalPrice, h.status_id AS orderStatus, h.method_payment AS paymentMethod, h.from_date AS startDate, h.to_date AS expiredDate FROM tb_histories h JOIN tb_users u ON u.id = h.user_id JOIN tb_users o ON o.id = h.owner_id JOIN tb_vehicles v ON v.id = h.vehicle_id WHERE h.user_id = ? OR h.owner_id = ?`;
